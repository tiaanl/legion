...
void createMaterial(ca::Renderer* renderer, le::ResourceManager* resourceManager,
                    const si::Material& src, Material* dst) {
  {
    ...
    if (!src.diffuse.texture.empty()) {
      auto image = resourceManager->get_image(src.diffuse.texture.view());
  ...
  if (dst->texture.id) {
    ...
    auto vertexShader = resourceManager->get_shader_source("diffuse_texture_v.glsl");
    auto fragmentShader = resourceManager->get_shader_source("diffuse_texture_f.glsl");
    ...
    auto vertexShader = resourceManager->get_shader_source("diffuse_lopoly_v.glsl");
    auto fragmentShader = resourceManager->get_shader_source("diffuse_lopoly_f.glsl");
...
Model Model::create_from_scene(si::Scene& scene, ResourceManager* resource_manager,
